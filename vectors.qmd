...
As we saw in the lesson on [iteration](iteration.html), you can also create vectors of sequences using the `:` operator or the `seq()` function:
...
As we saw in the lesson on [iteration](iteration.html), you can create a vector of integers using the `:` operator or the `seq()` function:
...
As we saw in the lesson on [iteration](iteration.html), you can use a loop to perform an operation on each element in a vector. For example, the following loop get the decimal values for each element in a vector of floats:
